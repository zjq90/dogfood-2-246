...
<script type="text/javascript">
    ...
    function loadFriends() {
        fetch('/api/friend/list?currentPage=' + currentPage + '&pageSize=' + pageSize)
    ...
    function renderUsers(pageData) {
        ...
        for(var i = 0; i < users.length; i++) {
            ...
            html += '<div class="user-signature">' + (user.signature || '') + '</div>';
            html += '</div>';
            html += '<div class="user-actions">';
            html += '<button onclick="sendDoumail(' + user.userId + ')">发豆邮</button>';
            html += '<button onclick="unfollowUser(' + user.userId + ')">删除好友</button>';
            html += '</div>';
            html += '</div>';
        }
        
        document.getElementById('user_list').innerHTML = html || '暂无好友';
